...
c_net = Net(threshold=8, use_cuda=True).cuda()
c_net.eval()

def canny(img, t=8, use_cuda=True):
    # batch = torch.stack([img]).float()    
    batch = img
    data = Variable(batch).cuda()
    with torch.no_grad():
        # blurred_img, grad_mag, grad_orientation, thin_edges, thresholded, early_threshold = c_net(data)
        early_threshold = c_net(data)
    return early_threshold
