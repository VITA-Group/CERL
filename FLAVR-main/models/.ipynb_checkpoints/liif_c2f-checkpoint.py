...
class LIIF_3d(nn.Module):
    ...
    def __init__(self, encoder_spec, imnet_spec=None,
        ...
        self.coarse_imnet = Siren(in_features=imnet_in_dim2, out_features=64, hidden_features=[64, 256, 256], 
    ...
    def query_rgb(self, coord, Training=True):
        ...
        if Training == True:
            for c in range(3):
                ...
                pe_coord = torch.ones_like(coord[c][:, :, -1].unsqueeze(2)) * (floor(time * 16) / 16)
                pe_coord2 = torch.ones_like(coord[c][:, :, -1].unsqueeze(2)) * (time - floor(time * 16) / 16)
                pe_coord3 = torch.ones_like(coord[c][:, :, -1].unsqueeze(2)) * (floor(time * 16) / 16 + 1)
                ...
                coarse_inp1 = torch.cat([q_feat, pe_coord], dim=-1)
                coarse_inp2 = torch.cat([q_feat, pe_coord3], dim=-1)
                
                feat_map1 = self.coarse_imnet(coarse_inp1.view(bs * qs, -1))[1].view(bs, qs, -1)
                feat_map2 = self.coarse_imnet(coarse_inp2.view(bs * qs, -1))[1].view(bs, qs, -1)
                feat_map = feat_map1 * (floor(time * 16) / 16 + 1 - time) + feat_map2 * (time - floor(time * 16) / 16)
                # feat_map = torch.cat([feat_map1, feat_map2], dim=-1)
                encode_inp = torch.cat([feat_map, pe_coord], dim=-1)
            ...
            time_idxs = np.argsort(np.abs(ref_time_stamps - time))[0]
            pe_coord = torch.ones_like(coord[:, :, -1].unsqueeze(2)) * (floor(time * 16) / 16)
            pe_coord2 = torch.ones_like(coord[:, :, -1].unsqueeze(2)) * (time - floor(time * 16) / 16)
            pe_coord3 = torch.ones_like(coord[:, :, -1].unsqueeze(2)) * (floor(time * 16) / 16 + 1)
            #pe_coord = self.position_encoding(time)[:, 0].repeat(q_feat.shape[0], q_feat.shape[1], 1).cuda()
            coarse_inp1 = torch.cat([q_feat, pe_coord], dim=-1)
            coarse_inp2 = torch.cat([q_feat, pe_coord3], dim=-1) 
            ...
            feat_map = feat_map1 * (floor(time * 16) / 16 + 1 - time) + feat_map2 * (time - floor(time * 16) / 16)
            # feat_map = torch.cat([feat_map1, feat_map2], dim=-1)
            encode_inp = torch.cat([feat_map, pe_coord2], dim=-1)
