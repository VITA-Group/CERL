...
class X4K_dataset(Dataset):
    def __init__(self, data_root , mode="train", interFrames=1, n_inputs=2, ext="png", random_seed = 0, seted_interval=1):
        ...
        for video in video_list:
            ...
            # interval = 7
            folders = sorted(os.listdir(os.path.join(self.data_root , video)))
            for folder in folders:
                frames = sorted(os.listdir(os.path.join(self.data_root , video, folder)))
                while index + interval * 3 + 3 < len(frames):
                    videoInputs_index = [index, index + 1 + interval, index + 2 + interval * 2, index + 3 + interval * 3]
                    videoInputs = [frames[i] for i in videoInputs_index]
                    video_all_gt = [frames[i] for i in range(index + 1 + interval, index + 3 + interval * 2)]
                    videoInputs = [os.path.join(video, folder, f) for f in videoInputs]
                    videoGts = [os.path.join(video, folder, f) for f in video_all_gt]
    ...
    def __getitem__(self, idx):
        ...
        images = [img_.resize((256, 256), Image.ANTIALIAS) for img_ in images]
        ...
        gt_images = [transforms.CenterCrop(512)(img_) for img_ in gt_images]
        gt_images = [img_.resize((256, 256), Image.ANTIALIAS) for img_ in gt_images]
        gt_images = [transforms.ToTensor()(img_) for img_ in gt_images]
...
def get_loader(mode, data_root, batch_size, shuffle, num_workers, random_seed, seted_interval, interFrames=7, n_inputs=4):
    ...
    dataset = X4K_dataset(data_root , mode, interFrames=interFrames, n_inputs=n_inputs, random_seed=random_seed, seted_interval=seted_interval)
