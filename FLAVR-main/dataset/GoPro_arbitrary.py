...
class GoPro(Dataset):
    def __init__(self, data_root , mode="train", interFrames=1, n_inputs=2, ext="png", random_seed=0, seted_interval=4):
        ...
        for video in video_list:
            video = video[:-1]
            index = 0
            interval_ind = np.random.randint(0, 22)
            interval = interval_ind % seted_interval + 14
            # interval = 7
    ...
    def __getitem__(self, idx):
        ...
        images = [img_.crop((x, y, x + crop_width, y + crop_height)) for img_ in images]
        gt_images = [img_.crop((x, y, x + crop_width, y + crop_height)) for img_ in gt_images]
        
        images = [img_.resize(((self.crop_size // 2), (self.crop_size // 2)), resample=Image.LANCZOS) for img_ in images]
