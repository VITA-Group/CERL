...
class GoPro(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        images = [transforms.CenterCrop(224)(img_) for img_ in images]
        # images = [img_.resize((112, 112), resample=Image.LANCZOS) for img_ in images]
        images = [self.transforms(img_) for img_ in images]

        gt_images = [Image.open(pth_) for pth_ in gt_paths]
        gt_images = [transforms.CenterCrop(224)(img_) for img_ in gt_images]
        gt_images = [self.transforms(img_) for img_ in gt_images] 
        
        h, w = gt_images[0].shape[1], gt_images[0].shape[2]
